...
export class ReplicatorTests extends TestCase {
  ...
  private async runReplication(config: ReplicatorConfiguration, reset: boolean = false,): Promise<void> {
    ...
    const completionPromise = new Promise<void>((resolve, reject) => {
      replicator.addChangeListener((change) => {
        ...
        console.log('PROGRESS:' ,change.status.getProgress())
  ...
  //       console.log(change.status.getProgress())
...
// }
  ...
  //   const replPushConfig = this.createConfig(ReplicatorType.PUSH, true, this.defaultCollection)
  ...
  //   function filterFn(doc) {
  //     'show source please'
  //     console.log("finally works: ",doc["name"])
  //     return !doc["name"].includes('not-pull');
  //   }
  //   collectionConfig.setPullFilter(filterFn)

  //   const replPullConfig = this.createConfig(ReplicatorType.PULL, true, this.defaultCollection,collectionConfig)
...
//         const docData = {
//           filterProperty: i % 3 === 0 ? 'type1' : i % 3 === 1 ? 'type2' : 'type3',
//           name: `Document ${i}`,
//         };
        
//         const mutableDoc = this.createDocumentWithIdAndData(docId, docData);
//         // mutableDoc.setArray('channels', ['channel.team1']);
...
//       return doc['filterProperty'] === 'type1';
//     });

//     const pushConfig = this.createConfig(ReplicatorType.PUSH, false, this.defaultCollection, collConfig);

//     await this.runReplication(pushConfig);

//     // Setup the filter configuration for pull replication
//     // collConfig.setChannels(['channel.team1']); 
//     return {
...
// }
 ...
 async testFilterPerformance(): Promise<ITestResult> {
  try {
    ...
    // Create dataset and push documents to the server
    const docIds = await prepareDocuments(count, "test");
    const pushConfig = this.createConfig(ReplicatorType.PUSH, false, this.defaultCollection);
...
// console.log('here')
  ...
  //     "SELECT COUNT(*) as count FROM _ WHERE filterProperty > 1000"
  //   );
  //   const result = await query.execute();
  //  const docCount =  await (await (await this.database.defaultCollection()).count()).count
  //   console.log(JSON.stringify(result[0].count));
  //   console.log({docCount})
//     console.log(Math.ceil(count / 3));
    

    // expect(Math.ceil(count / 3)).to.be.equal(result[0].count); 
