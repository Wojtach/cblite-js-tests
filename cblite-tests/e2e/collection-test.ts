...
import {ConcurrencyControl, Database, MutableDocument} from 'cblite-js';
        ...
        try {
            token1 = await colA.addChangeListener((change) => {
                ...
                if (change.collection.name === 'colA') {
                    ...
                    throw new Error("CollectionB shouldn't trigger CollectionA's listener");
                }
            });
        } catch (err) {
            throw err;
...
async testCollectionChangeListenerWithoutRemoveToken(): Promise<ITestResult> {
    try {
        ...
        await colA.addChangeListener((change) => {
            console.log(`Listener triggered with ${change.documentIDs.length} document IDs`);
